...
def emotion_detector(model, cam_id=0):
    ...
    while True:
        ...
        for rect in rects:
            ...
            preds = model.predict((landmark_vect))
            print(landmark_vect)
            emotion_probability=np.max(preds)
            text="{}: {:.2f}%".format(emotions[emotion_idx], emotion_probability*100)
            # print(text)
            ...
            # print(text,emotions[emotion_idx])

            # loop over the (x, y)-coordinates for the facial landmarks and draw them on the image
            for (x, y) in coords:
                cv2.circle(frame, (x, y), 1, (0, 0, 255), -1)
        cv2.imshow('Landmark Facial Emotion Detector', frame)
...
# Model Loading
fer_model = load_model("simple DNN.74-0.54.hdf5")

# CV2 function to show the real-time testing
